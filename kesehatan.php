...
// Build SQL query - ONLY for health category
if (!empty($search_query)) {
    $search_term = '%' . $conn->real_escape_string($search_query) . '%';
    $sql = "SELECT id, title, emoji, image, organizer, target_terkumpul, donasi_terkumpul 
            FROM campaigns 
            WHERE category = 'kesehatan' AND (title LIKE ? OR organizer LIKE ?)
    ...
    // Show all health campaigns if no search query
    $sql = "SELECT id, title, emoji, image, organizer, target_terkumpul, donasi_terkumpul 
            FROM campaigns 
...
<head>
    ...
    <title>Kampanye Kesehatan - KolaborAksi</title>
    ...
    <style>
        * {
        ...
        body {
            ...
            background: linear-gradient(135deg, #FFF5F5 0%, #FFE8E8 25%, #FFFFFF 50%, #FFE8E8 75%, #FFF5F5 100%);
...
<body>
    <div class="header">
        <div class="header-content">
            ...
            <h1 class="page-title">❤️ Kampanye Kesehatan</h1>
        ...
        <div class="search-section">
            <form method="GET" action="" class="search-form">
                <input 
                    ...
                    name="q" 
                    class="search-input" 
                    placeholder="Cari kampanye kesehatan..."
        ...
        <div class="results-info">
            <div class="results-count">
                <?php if (!empty($search_query)): ?>
                    Ditemukan <strong><?php echo $total_results; ?></strong> kampanye kesehatan untuk "<strong><?php echo htmlspecialchars($search_query); ?></strong>"
                <?php else: ?>
                    Menampilkan <strong><?php echo $total_results; ?></strong> kampanye kesehatan
                <?php endif; ?>
            </div>
            <?php if (!empty($search_query)): ?>
            <a href="kampanye-kesehatan.php" class="clear-search">
            ...
            <a href="kampanye-detail.php?id=<?php echo $campaign['id']; ?>" class="campaign-card">
                <div class="campaign-image">
                    <?php if (!empty($campaign['image'])): ?>
                        <img src="<?php echo htmlspecialchars($campaign['image']); ?>" alt="<?php echo htmlspecialchars($campaign['title']); ?>" onerror="this.parentElement.innerHTML='<?php echo $campaign['emoji'] ?: '❤️'; ?>'">
                    <?php else: ?>
                        <?php echo $campaign['emoji'] ?: '❤️'; ?>
                    <?php endif; ?>
                </div>
        ...
        <div class="empty-state">
            ...
            <h2 class="empty-title">
                <?php if (!empty($search_query)): ?>
                    Tidak Ada Hasil
                <?php else: ?>
                    Belum Ada Kampanye Kesehatan
                <?php endif; ?>
            </h2>
            <p class="empty-text">
                <?php if (!empty($search_query)): ?>
                    Maaf, kami tidak menemukan kampanye kesehatan yang cocok dengan pencarian "<strong><?php echo htmlspecialchars($search_query); ?></strong>".<br>
                    Coba kata kunci lain atau <a href="kampanye-kesehatan.php" style="color: #ef4444; font-weight: 600;">lihat semua kampanye kesehatan</a>.
                <?php else: ?>
                    Belum ada kampanye kesehatan yang tersedia saat ini.<br>
